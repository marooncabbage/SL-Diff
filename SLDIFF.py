...
from main.model.GMT import gmtnets
from main.model.SLDIFFnet import dualenc
import argparse

from main.utils import load_dataset, InverseProblemDataset, adj_process
from main.model.model import GNNModel, diffusionModel, DisseminationPropagate, Encoder, Decoder
...
parser = argparse.ArgumentParser(description="SLDIFF")
datasets = ['Digg', 'Memetracker', 'Android', 'Christianity', 'Twitter', 'power_grid_SI',
            ]
...
diffusion_model = dualenc.DualEncoderEpsNetwork(Encoder=encoder, Decoder=decoder)


gnn_model = gmtnets.GraphRepresentation(input_dim=5,
                     hiddenunits=[64, 64], 
                     num_classes=1, 
                     prob_matrix=prob_matrix)


propagate = DisseminationPropagate(prob_matrix, niter=2)




model = InverseModel(diffusion_model, gnn_model, propagate).to(device)
...
for epoch in range(100):
    ...
    for batch_idx, data_pair in enumerate(train_set):
        ...
        y_hat_copy=y_hat.cpu().detach()
        print ("x_pred.shapeeeeeee:", x_pred.shape)
        #save_image(x_pred,'./x_pred.png')
        show_images(x_pred,f"./pics/xpred_epoch{epoch}_batchidx{batch_idx}.png")
        show_images(y_hat_copy,f"./pics/yhat_epoch{epoch}_batchidx{batch_idx}.png")
...
decoder = diffusion_model.Decoder
...
for test_id, test in enumerate(test_set):
    ...
    with torch.no_grad():
        ...
        z_all = diffusion_model.reparameterization(mean, var)
